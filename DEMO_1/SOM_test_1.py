...
for t in range(N):
	...
	u = np.array((float(data.iloc[n-1][0]), float(data.iloc[n-1][0]), float(data.iloc[n][2]), float(data.iloc[n][3]), float(data.iloc[n][4])))

	# Similarity matching OR Finding minimum distance
	dist_min = BMU(W[0,0,0], W[0,0,1], W[0,0,2], W[0,0,3], W[0,0,4], float(data.iloc[n-1][0]), float(data.iloc[n-1][1]), float(data.iloc[n][2]), float(data.iloc[n][3]), float(data.iloc[n][4]))
	i_min = 0
	j_min = 0

	for i in range(I):
		for j in range(J):
			dist = BMU(W[i,j,0], W[i,j,1], W[i,j,2], W[i,j,3], W[i,j,4], float(data.iloc[n-1][0]), float(data.iloc[n-1][1]), float(data.iloc[n][2]), float(data.iloc[n][3]), float(data.iloc[n][4]))
			if(dist < dist_min):
				dist = dist_min
				i_min = i
				j_min = j
