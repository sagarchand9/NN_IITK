...
J = 5
N = 5000
#sigma_0 = float(max(I,J))/(1000*float(2))
sigma_0 = 1
L_0 = 0.01
...
data.drop(data.index[10], axis=1, inplace=True)
data.drop(data.index[11], axis=1, inplace=True)
data.drop(data.index[12], axis=1, inplace=True)
...
def BMU(a1,a2,a3,a4,a5,a6,a7,b1,b2,b3,b4,b5,b6,b7):
	return math.sqrt(pow((a1-b1),2) + pow((a2-b2),2) + pow((a3-b3),2) + pow((a4-b4),2) + pow((a5-b5),2) + pow((a6-b6),2) + pow((a7-b7),2))	
	
# creating and initialising random weight matrix 
W = np.random.rand(I,J,7) 
W = W*2 - 1
Theta = np.random.rand(I,J,2)
Theta = Theta*2 - 1
A = np.random.rand(I,J,2,7) 
A = A*2 - 1
...
for t in range(N):
	...
	u = np.array((float(data.iloc[n-2][0]), float(data.iloc[n-2][1]), float(data.iloc[n-1][0]), float(data.iloc[n-1][1]), float(data.iloc[n][2]), float(data.iloc[n][3]), float(data.iloc[n][4])))

	# Similarity matching OR Finding minimum distance
	dist_min = BMU(W[0,0,0], W[0,0,1], W[0,0,2], W[0,0,3], W[0,0,4], W[0,0,5], W[0,0,6], float(data.iloc[n-2][0]), float(data.iloc[n-2][1]), float(data.iloc[n-1][0]), float(data.iloc[n-1][1]), float(data.iloc[n][2]), float(data.iloc[n][3]), float(data.iloc[n][4]))
	i_min = 0
	j_min = 0

	for i in range(I):
		for j in range(J):
			dist = BMU(W[i,j,0], W[i,j,1], W[i,j,2], W[i,j,3], W[i,j,4], W[i,j,5], W[i,j,6], float(data.iloc[n-2][0]), float(data.iloc[n-2][1]), float(data.iloc[n-1][0]), float(data.iloc[n-1][1]), float(data.iloc[n][2]), float(data.iloc[n][3]), float(data.iloc[n][4]))
			if(dist < dist_min):
				dist = dist_min
				i_min = i
				j_min = j
	...
	sigma = sigma_0*(math.exp(-(float(t)/float(lamda))))

	# Learning rate
	L = L_0*(math.exp(-(float(t)/float(lamda))))
	# L=L_0
	# L = L_0*(10*(N-t))/N
	...
	for i in range(I):
		for j in range(J):
			...
			h[i,j] = (math.exp(-(float(d2)/float(2*sigma*sigma))))
			sum_h = sum_h + h[i,j]
